...
function rebuildFromFile(filePath) {
...
function writeTemplatesFile(entries, filePath) {
  const lines = [];
  lines.push('// Auto-generated by rebuild-templates.mjs — do not edit manually');
  lines.push('// Run: node scripts/rebuild-templates.mjs');
  lines.push('');
  lines.push('export const opTemplates = {');
  for (const e of entries) {
    ...
    const rs = esc(e.rs || '');
    const go = esc(e.go || '');
    lines.push(`  '${e.name}':     { js: '${js}',                           py: '${py}',                           rs: '${rs}',                          go: '${go}' },`);
  }
  lines.push('};');
  lines.push('');
...
// Verify
const verify = readFileSync('src/templates.js', 'utf-8');
const vNames = [...verify.matchAll(/'(\w+)':/g)].map(x => x[1]);
let missRS = 0, missGO = 0;
for (const name of vNames) {
  const idx = verify.indexOf(`'${name}':`);
  const snippet = verify.slice(idx, idx + 500);
  if (!snippet.includes('rs:')) missRS++;
  if (!snippet.includes('go:')) missGO++;
}
console.log(`  verified: ${vNames.length} total, missing rs: ${missRS}, missing go: ${missGO}`);
