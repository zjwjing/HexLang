...
Hex64 的64卦由8个经卦（三爻卦）两两叠加而成。以下是基础单元的映射：

| 卦 | 爻 | 二进制 | 八进制 | HexLang 语义标签 |
|----|----|--------|--------|----------------|
| 乾☰ | ███ | 111 | 7 | 初始化、核心、调度、创建 |
| 兑☱ | ██░ | 110 | 6 | 订阅、监听、代理、通知 |
| 离☲ | █░█ | 101 | 5 | 可视化、展示、交付、渲染 |
| 震☳ | █░░ | 100 | 4 | 启动、触发、重构、事件 |
| 巽☴ | ░█░ | 011 | 3 | 探索、查询、同步、遍历 |
| 坎☵ | ░░█ | 010 | 2 | 存储、缓存、容错、数据源 |
| 艮☶ | ░██ | 001 | 1 | 暂停、等待、限流、休眠 |
| 坤☷ | ░░░ | 000 | 0 | 终止、复位、归档、稳定 |

> 64卦 = 8经卦 × 8经卦（上卦 × 下卦），6位二进制 = 2³ + 2³ = 2⁶ = 64
