...
基于 Hex64 编码的运维监控可视化工具，将 6 项系统指标映射为 6 位二进制卦象，通过互卦趋势分析和爻权重预警实现健康度感知。

```bash
# 启动仪表盘
python -m http.server 8080
# 访问 http://localhost:8080/src/dashboard/index.html
```

| 指标 | 爻位 | 阈值 | 说明 |
|------|------|------|------|
| CPU 使用率 | 初爻 | 80% | 基础算力负载 |
| 内存使用率 | 二爻 | 75% | 内存资源占用 |
| 磁盘 IO | 三爻 | 70% | 存储读写压力 |
| 网络延迟 | 四爻 | 100ms | 网络传输状态 |
| 错误率 | 五爻 | 1% | 业务错误率 |
| 运行时长 | 上爻 | 30天 | 系统运行时长 |
